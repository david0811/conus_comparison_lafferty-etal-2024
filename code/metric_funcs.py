...
def calculate_cdd(tasmin, tasmax, b):
    ...
    cdd_thresh_below = (tasmax + tasmin) / 2. - b
    cdd_thresh_between = (t_bar / np.pi) * ((tasmax + tasmin) / 2 - b) + (tasmax - tasmin) / (2 * np.pi) * np.sin(t_bar)
    cdd_thresh_above = 0.
    
    return np.where(b <= tasmin, cdd_thresh_below, np.where(b >= tasmax, cdd_thresh_above, cdd_thresh_between))

# ufunc for dask
def cdd_ufunc(tasmin, tasmax, threshold = thresh_c):
    return xr.apply_ufunc(calculate_cdd,
                          tasmin, tasmax,
                          threshold,
                          dask = 'allowed')
...
def calculate_hdd(tasmin, tasmax, b):
    ...
    hdd_thresh_below = 0.
    hdd_thresh_between = (1 - t_bar / np.pi) * (b - (tasmax + tasmin) / 2) + (tasmax - tasmin) / (2 * np.pi) * np.sin(t_bar)
    hdd_thresh_above = b - (tasmax + tasmin) / 2.
...
def hdd_ufunc(tasmin, tasmax, threshold = thresh_c):
    return xr.apply_ufunc(calculate_hdd,
                          tasmin, tasmax,
                          threshold,
                          dask = 'allowed')

# Degree days
def calculate_dd(ds_in, var_id):
    # CDD
    if var_id == 'cdd':
